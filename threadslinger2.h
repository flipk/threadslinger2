...
\code
class MyMessage1 : public ThreadSlinger2::t2t_message_base<MyMessage1>
{
public:
   // convenience
   typedef ThreadSlinger2::t2t_shared_ptr<MyMessage1> sp_t;
   // etc
};
\endcode
 */
...
public:
     ...
     *           <ul> <li> -2 = GROW : grow pool if empty </li>
     *                <li> -1 = WAIT_FOREVER : wait forever </li>
     *                <li> 0 = NO_WAIT : don't wait at all, if the
...
public:
     ...
     * \param wait_ms  how long to wait: \ref wait_flag
     *           <ul> <li> -1 = WAIT_FOREVER : wait forever </li>
     *                <li> 0 = NO_WAIT : don't wait at all, if the
     ...
     *       queue has been added to a t2t_queue_set. that will throw
     *       an assertion. */
...
public:
     ...
     * \param q the t2t_queue to add to this set. the queue must be
     ...
     * \param wait_ms  how long to wait: \ref wait_flag
     *           <ul> <li> -1 = WAIT_FOREVER : wait forever </li>
     *                <li> 0 = NO_WAIT : don't wait at all, if all the
...
Classes of interest provided by this library:
 ...
 <li> \ref ThreadSlinger2::t2t_queue_set
...
\section Rules Rules
   ...
   <li> All messages must derive from t2t_message_base<> template. They
        are allowed to derive in multiple levels, but t2t_message_base
...
\code
namespace ts2 = ThreadSlinger2; // convenience alias
class my_message_base : public ts2::t2t_message_base<my_message_base>
{
public:
    // convenience
    typedef ts2::t2t_pool      <my_message_base> pool_t;
...
\note the arguments to the t2t_pool template are a variable-length
...
\code
    ...
    // here just to demonstrate the t2t_queue_set functionality.
...
\note the "sp_t" type is a shortcut typedef for t2t_shared_ptr defined
   above.  also note the enqueue "takes" the value out of the
   t2t_shared_ptr, so the "spmb" is now empty (NULL).
...
\ref ThreadSlinger2::t2t_shared_ptr has a helpful facility for
...
ThreadSlinger2 errors, you can register a function to handle them
using the global variable \ref ThreadSlinger2::ts2_assert_handler.

You can also access printable string descriptions of the error enums
via the global array \ref ThreadSlinger2::ts2_error_types, as long
as the enum value is less than T2T_NUM_ERRORS.
...
{
   ThreadSlinger2::ts2_assert_handler = &my_ts2_assert_handler;
}
\endcode
